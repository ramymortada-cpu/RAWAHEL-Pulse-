...
Auth: OAuth callback/session cookie flow, jose JWTs, production JWT_SECRET guard, local super_admin fallback only outside production, server-side RBAC via permissions.ts and tRPC middleware. Missing: MFA policy, rate limiting, formal account lockout, and IdP policy evidence.

## Required Coverage Checklist

- Login/OAuth/session: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Cookies/tokens: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- RBAC/permissions: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Admin access: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Brute force/abuse: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Missing controls: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Hardening roadmap: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
