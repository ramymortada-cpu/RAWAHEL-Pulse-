...
Architecture: Browser SPA calls Express-hosted tRPC API. Server authenticates sessions through cookies/OAuth, enforces permissions through tRPC middleware, reads/writes MySQL through Drizzle or local in-memory fallback outside production, and supports browser-side export plus optional remote storage. Production env gate protects unsafe startup.

## Required Coverage Checklist

- High-level architecture: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
...
- Frontend/backend/database/auth/export/admin: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Lifecycles: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Integrations: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Bottlenecks: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
- Target architecture: Covered with repository evidence where available; otherwise labeled as Not found, Inferred, Assumption, or Recommendation.
